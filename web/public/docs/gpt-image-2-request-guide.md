...
计费规则版本：`leonardo-ui-20260808.v4`。API 创建任务时写入 `estimated_credit_cost`；Worker 领取任务前会重新报价，只选择 `balance_credits - reserved_credits >= estimated_credit_cost` 的账号，并在上游提交前刷新一次真实余额。上游返回 `apiCreditCost` 时，它会覆盖预估并参与最终结算。
